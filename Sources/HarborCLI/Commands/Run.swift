...
extension Run {
    struct Web: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            // Step 1: Generate full Tailwind class list for dev (all classes, no tree-shaking).
            // Dev mode uses the full dump for zero-friction iteration — add a new TW class and
            // Tailwind watch picks it up immediately without restarting harbor.
            // (prod tree-shaking happens in `harbor build web` via extractClassesFromSource)
            if tailwind.isFleetTailwindPresent() {
                print("Generating full Tailwind class list for dev mode...")
                do {
                    try tailwind.generateFullClassList()
                } catch {
                    print("Warning: Could not generate Tailwind class list: \(error). Continuing with existing file if present.")
                }
            }

            // Step 2: Build WASM target via PackageToJS
            let sdk = SwiftSDK.resolveWasmSDK()
            print("Building WASM target with Swift SDK '\(sdk)'...")
            let buildOk = try await shellCommand(
                "/usr/bin/env",
                arguments: ["swift", "package", "--swift-sdk", sdk, "js"],
            ...
            // Step 3: Tailwind CSS integration (if Fleet-Tailwind is a dependency)
            var tailwindProcess: Process? = nil

            if tailwind.isFleetTailwindPresent() {
                if tailwind.hasTailwindBinary {
                    // Start Tailwind in watch mode (class list was already written above)
